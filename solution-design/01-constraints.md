...
| 4 | SIGTERM during active relay | Main thread receives SIGTERM while relay loop is blocking on `zmq_msg_recv` | Graceful — the shutdown pattern sends a dummy message to unblock recv. Threads must join within 3 seconds. | No operator action. systemd logs clean shutdown. If the join hangs past 3 seconds, systemd sends SIGKILL — the operator sees `main process exited, code=killed, status=9/KILL` in journal. This is a reliability concern the conversion should address. |
